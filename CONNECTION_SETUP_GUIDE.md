...
| 자동매핑으로 초기화 | 직접 지정한 컬럼 매핑을 모두 지우고 자동매핑 결과로 되돌림 |
...
그 항목은 화면의 매핑 결과에 `❌ 없음`으로 표시된다.

### 컬럼 매핑 직접 지정

자동매핑은 이름만 보고 판단하므로 틀릴 수 있다. 예를 들어 테이블에 레코드 상태
컬럼 `STATUS`와 서버 운영상태 컬럼 `OPER_STATUS`가 함께 있으면 `CM_STA_CD`는
`STATUS`를 선택한다. 이 경우 매핑 결과 표에는 `🔁 자동매핑`으로 정상처럼 보이지만
값은 틀린 상태다.

매핑 결과 표의 `원본 컬럼`은 선택 상자이며, 대상 테이블의 실제 컬럼 중에서 직접
고를 수 있다. `(NULL로 조회)`를 선택하면 자동매핑을 무시하고 해당 컬럼을 비운다.
변경 후 `매핑 미리보기` 또는 `자산 원본으로 적용`을 다시 눌러야 SQL에 반영된다.
직접 지정한 항목은 `✏️ 직접 지정`으로 표시된다.

### 조회 조건

조회 조건을 지정하지 않으면 대상 테이블의 **전체 행**을 수집한다. 서버 자산만
수집하려면 조회 조건 컬럼과 코드값을 지정한다.

```text
조회 조건 컬럼: CM_CAT_CD
코드값:        HW0101, HW0102, HW0104
→ WHERE CM_CAT_CD IN ('HW0101', 'HW0102', 'HW0104')
```

코드값은 사이트마다 다르므로 자동으로 채우지 않는다. 실제와 다른 값을 넣으면
0건 조회가 되고 수집기가 정상 스냅샷으로 저장하지 않으므로, 위 화면의 샘플
데이터에서 실제 코드값을 먼저 확인한다.

생성된 `config/oracle_query.local.sql`은 직접 수정해도 된다. 이 파일은
`.gitignore` 대상이며 폐쇄망 서버에만 존재한다.
