...
def main() -> None:
    parser = argparse.ArgumentParser(description="Collect vCenter inventory through PowerCLI")
    parser.add_argument("--mode", choices=["DEMO", "FILE_ONLY", "POWERCLI"])
    args = parser.parse_args()
    cfg = load_config()
    configure_logging(cfg.resolve("logs"), cfg.log_level, cfg)
    manager = SQLiteManager(cfg.database_path)
    manager.initialize()
    result = CollectionService(cfg, manager).collect_vcenter(mode=args.mode)
